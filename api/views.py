...
from .models import (
    ...
    Category, Location, Inventory, InventoryMovement, Buyer, Supplier # Ensure Buyer and Supplier are imported
)
...
class OrganizationOnboardingView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        # --- Add logic to create Buyer/Supplier profile based on organization type ---
        if organization.organization_type in ['buyer', 'both']:
            # Create a Buyer profile for the initial user in a buyer or 'both' organization
            # You might want to populate more fields here if available in the onboarding data
            buyer_profile, created = Buyer.objects.get_or_create(
                ...
                defaults={
                    ...
                    'last_name': user.last_name,
                    'email': user.email,
                    'name': organization.name, # Use organization name as buyer name initially
                    # buyer_code will be generated by the Buyer model's save method
                }
            )
            if created:
                print(f"Created Buyer profile for user {user.email} in organization {organization.name}")
            else:
                print(f"Buyer profile already exists for user {user.email} in organization {organization.name}")


        if organization.organization_type in ['supplier', 'both']:
            # Create a Supplier profile for the organization if it's a supplier or 'both' type
            # Note: The Supplier model doesn't have a direct user link currently.
            # We link it to the organization.
            supplier_profile, created = Supplier.objects.get_or_create(
                organization=organization,
                defaults={
                    'name': organization.name,
                    # supplier_code will be generated by the Supplier model's save method
                    'email': organization.contact_email, # Use organization contact email
                    'phone': organization.contact_phone, # Use organization contact phone
                    # Add other default fields as needed
                }
            )
            if created:
                 print(f"Created Supplier profile for organization {organization.name}")
            else:
                 print(f"Supplier profile already exists for organization {organization.name}")
        # --- End Buyer/Supplier profile creation logic ---
